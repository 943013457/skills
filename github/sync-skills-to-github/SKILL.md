...
上传前必须扫描 skill 目录下**所有文件**（不仅仅是 SKILL.md），检测是否有敏感信息。发现敏感信息时必须先提示用户确认，用户同意后才能继续上传。

#### 3a. 高风险文件类型（必须检查）

以下文件类型即使不在标准扫描规则内，也必须人工审核：
- `config.json` / `config.yaml` / `config.toml` — 通常含 API Key
- `.env` / `.env.local` / `.env.production` — 环境变量文件
- `*.key` / `credentials.json` — 密钥文件
- `secrets.json` / `secrets.yaml` — 密钥配置文件
- `token.json` — Token 存储文件
- `package-lock.json` — Node 依赖锁文件（可能含敏感字段）
...
SENSITIVE_PATTERNS = [
    ...
    (r'[a-f0-9]{32}', 'Generic 32-char Hex (可能是 API Key/Token)'),
    (r'[A-Za-z0-9]{40}', 'Generic 40-char String (可能是 AWS Key 或 Token)'),
]

# 高德地图 WebService Key 专用规则
AMAP_KEY_PATTERN = re.compile(r'[a-f0-9]{32}', re.IGNORECASE)

def scan_sensitive(content: str, filename: str = "") -> list:
    """扫描敏感信息，返回匹配列表 [(pattern_name, matched_text, context)]"""
    findings = []
    for pattern, name in SENSITIVE_PATTERNS:
        matches = re.findall(pattern, content, re.IGNORECASE)
        for m in matches:
            masked = m[:8] + '...' + m[-4:] if len(m) > 16 else m[:4] + '...'
            findings.append((name, masked, f"文件: {filename}"))

    # 高德地图 Key 特例：32位十六进制字符串
    if filename.endswith(('config.json', 'config.yaml', '.env')):
        for match in AMAP_KEY_PATTERN.findall(content):
            if len(match) == 32 and re.match(r'^[a-f0-9]{32}$', match, re.I):
                masked = match[:8] + '...' + match[-4:]
                findings.append(('高德地图 API Key (32位hex)', masked, f"文件: {filename}"))

    return findings
```

#### 3c. 扫描范围：skill 目录下所有文件

**错误做法**：只扫描 SKILL.md
**正确做法**：扫描 skill 目录下每一个文件

```python
import os

def scan_skill_directory(skill_path: str) -> list:
    """
    扫描整个 skill 目录，返回所有敏感信息发现
    路径示例: /home/hermeswebui/.hermes/skills/amap-lbs-skill/
    """
    all_findings = []
    ...
    for root, dirs, files in os.walk(skill_path):
        ...
        for filename in files:
            ...
            # 跳过二进制文件和大型文件
            if filename.endswith(('.png', '.jpg', '.gif', '.zip', '.tar', '.gz', '.exe')):
                continue
            ...
            findings = scan_sensitive(content, filename=rel_path)
...
```python
all_findings = scan_skill_directory(skill_path)

if all_findings:
    print("⚠️  检测到以下敏感信息：")
    for name, masked, context in all_findings:
        print(f"  - [{name}] {masked} ({context})")

    response = clarify(
        question=f"检测到 {len(all_findings)} 处敏感信息，继续上传可能存在安全风险。是否仍要继续上传？",
        choices=["继续上传（已确认无风险）", "取消上传"]
    )
...
- 不在日志/输出中保留完整敏感值
- 发现后立即暂停，等待用户确认
- **配置文件（config.json/.env 等）必须单独扫描，不能只看 SKILL.md**
...
- **GitHub 安全规则**：包含真实 token 的文件上传会被拒绝（409 Conflict），先扫描敏感信息
- **node_modules**：包含大量依赖的 skill 只上传核心文件，不传 node_modules/

## 常用 Skill 路径参考

| Skill | 本地路径 |
|-------|---------|
| find-docs | `/home/hermeswebui/.hermes/skills/productivity/find-docs/SKILL.md` |
| amap-lbs-skill | `/home/hermeswebui/.hermes/skills/amap-lbs-skill/` |
| dingtalk-notify | `/home/hermeswebui/.hermes/skills/productivity/dingtalk-notify/` |
| xiaohongshu-read | `/home/hermeswebui/.hermes/skills/social-media/xiaohongshu-read/` |
| create-skill-hermes | `/home/hermeswebui/.hermes/skills/software-development/create-skill-hermes/` |
