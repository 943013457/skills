...
async function main() {
  ...
  // 确保有 Key（从环境变量或 config.json 读取）
  const { ensureWebServiceKey } = require('../index');
  try {
    await ensureWebServiceKey();
  } catch (e) {
    console.error('❌', e.message);
    console.log('\n设置 Key 的方式:');
    console.log('1. 环境变量: export AMAP_KEY=your_key');
    console.log('2. 或编辑 config.json 写入 webServiceKey');
    process.exit(1);
  }
