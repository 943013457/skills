...
name: amap-lbs-skill
description: >-
  当用户提到[附近/周边/附近有什么/周边有什么/附近好吃的/附近美食/周边美食/附近酒店/周边酒店/附近景点/周边景点]或明确要求[搜索/找/查/导航到/怎么去]某个[地点/位置/地址/地标]时，触发此 Skill 完成高德地图查询；
  关键词覆盖：[美食/餐厅/好吃/小吃/粉/粥/火锅/烧烤/酒店/住宿/景点/公园/超市/银行/加油站/停车场/地铁站/公交站] + [附近/周边/附近有什么/周边有什么/哪里有/哪个近/怎么走/怎么去/导航/路线/规划] + [我在这里/在这个位置/在这附近/我现在在/定位/坐标]（可中英混合）；
  英文增强：Trigger when user asks "nearby food/restaurants/hotels", "what's around/nearby", "find restaurants/hotels near", "how to get to", "directions to", "POI search", "Amap search", "search nearby";
  输入需要：[位置词（如"三十三中""南湖""万象城"）+ 搜索类别词（如"美食""酒店""景点"）]；输出为：[店铺列表/地址/距离/链接]；
  不适用于：[全国/全球范围搜索/不需要地理位置的场景（如纯计算/纯对话）]。
  典型用户表达："南宁三十三中附近什么好吃的" "找一下这附近美食" "周边有什么酒店" "从这里怎么去南湖" "万象城附近有什么好玩的" "帮我导航到" "搜索南湖周边停车场"
version: 2.0.0
metadata:
  openclaw:
    requires:
...
1. **环境变量**：`export AMAP_KEY=your_key` 或 `export AMAP_WEBSERVICE_KEY=your_key`
2. **config.json**：直接写入 `config.json` 的 `webServiceKey` 字段（脚本自动读取）
3. **命令行参数**：仅限 `node index.js your_key`

> ⚠️ **已知问题**：部分旧脚本（如 `poi-search.js`）在检查 Key 时没有调用 `ensureWebServiceKey()`，导致不读 config.json。已修复，现在所有脚本均支持从 config.json 自动读取 Key。

> 📦 **依赖**：脚本依赖 `axios`，如果提示 `Cannot find module 'axios'`，请先运行：
> ```bash
> cd /home/hermeswebui/.hermes/skills/amap-lbs-skill && npm install axios
> ```
