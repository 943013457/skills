...
## Fallback: GitHub API + Python urllib

When ctx7 fails (no API key for `--research`, quota exceeded, or ctx7 not in PATH),
fall back to fetching raw files directly from GitHub using Python:
...
url = "https://raw.githubusercontent.com/nousresearch/hermes-agent/main/Dockerfile"
...
- **`npm install -g ctx7@latest` may put `ctx7` outside PATH**: Always use `npx ctx7@latest` instead of relying on the global binary being found.
- **`--research` requires Context7 API key**: If not available, fall back to GitHub API + urllib.
- **Browser tool may fail with "env: 'node': No such file"**: The browser tool depends on Node.js in the environment. If unavailable, use Python urllib to fetch GitHub content instead.
